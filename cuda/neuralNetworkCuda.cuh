...
__device__ struct NeuralNetworkCuda {
  ...
  __device__ void PropagateBack(int i, int batch, int start,
                                int end) // i- layer index
  {
    int numberIndex = 0;
    float gradient;
    ...
    for (int jj = start; jj < end; jj++) {
      ...
      // Output ლეიერი
      if (i == LayersSize - 1)
        Layers[i].Outputs[bj] = DifferentiateLossWithCuda(
            Layers[i].Outputs[bj], Layers[i].Targets[bj], LossFunctionType,
            Layers[i].Size);
      else {
        ...
        for (int ll = Layers[i + 1].IndexVectorSize; ll--;) {
          ...
          Layers[i].Outputs[bj] +=
              Layers[i + 1].GetInputsBatch(batch, l) *
              Layers[i + 1]
      ...
      Layers[i].Inputs[bj] =
          Layers[i].Outputs[bj] *
          DifferentiateWithCuda(
              Layers[i].Inputs[bj], Layers[i].ActivationFunction,
              Layers[i].GetInputsBatch(batch), Layers[i].DropoutNeurons);
      ...
      for (int pp = Layers[i - 1].IndexVectorForNextLayerSize; pp--;) {
        ...
        if (BatchSize == 1)
          // Layers[i].Gradients[numberIndex] = Layers[i].Inputs[j] * Layers[i -
          // 1].Outputs[p];// ... if gradient optimization is needed
          Layers[i].Weights[numberIndex] -=
              Layers[i].Inputs[bj] *
              Layers[i - 1].Outputs[batch * Layers[i - 1].Size + p] *
  ...
  __device__ void PropagateBackCalculateGradients(
      int i, int batchCount, int weights_start,
      int weights_end) // i- layer index start და end წონის ვექტორის ქაუნთერია;
  {
    int numberIndex = 0;
    float gradient = 0;
    ...
    for (int wi = weights_start; wi < weights_end; wi++) {
      cli =
          Layers[i].IndexVector[wi / Layers[i - 1].IndexVectorForNextLayerSize];
      pli = Layers[i - 1].IndexVectorForNextLayer
                [wi % Layers[i - 1].IndexVectorForNextLayerSize];
      for (size_t b = 0; b < batchCount; b++) {

        gradient += Layers[i].Inputs[i_b * b + cli] *
                    Layers[i - 1].Outputs[ip_b * b + pli];
      }
      gradient /= batchCount;
      int idx = pLS_ * (cli - biasShift_) +
  ...
  __device__ void StartTesting() {
    for (int k = 0; k < LayersSize; k++) {
      if (Layers[k].LayerType ==
              static_cast<int>(NeuralEnums::LayerType::HiddenLayer) &&
          Layers[k].DropOutSize > 0) {
  ...
  __device__ float CalculateLoss(bool &training) {
    ...
    if (BatchSize > 1 && training) {
      ...
      for (int i = 0; i < ThreadCount; i++) {
        ...
        // workers[i]->doAsync(std::bind(&NeuralNetwork::CalculateLossBatchSub,
        // this, idx, idx + chunkSize, std::ref(losses[i])));
      }
    ...
    if (BatchSize == 0 || training) {
      ...
      if (Type == static_cast<int>(NeuralEnums::NetworkType::AutoEncoder) &&
          AutoEncoderType ==
              static_cast<int>(NeuralEnums::AutoEncoderType::Sparce)) {
      ...
      for (int i = 0; i < iterator; i++) {
        ...
        // start, end, klbstart, klbend, std::ref(lossesTmp[i])));
      }
  ...
  __device__ void CalculateLossBatchSub(int start, int end, float &loss) {
    ...
    for (size_t i = start; i < end; i++) {
      result += CalculateLossFunctionCuda(
          LossFunctionType, Layers[LayersSize - 1].GetOutputsBatch(i),
          Layers[LayersSize - 1].GetTargetsBatch(i), 0,
          Layers[LayersSize - 1].Size, Layers[LayersSize - 1].Size);
    }
    loss = result;
  }

  __device__ float GetLearningRateMultipliedByGrad(float &gradient,
                                                   int &iterator, int &j) {
    ...
    //	return AdaDelta(nn.Layers[iterator].GradientsLR,
    // nn.Layers[iterator].Parameters, gradient, j) * gradient; else if
    // (nn.LearningRateType == NeuralEnums::LearningRateType::Adam) 	return
    // Adam(nn, gradient, j, iterator); else if (nn.LearningRateType ==
    // NeuralEnums::LearningRateType::AdaMax) 	return AdaMax(nn, gradient, j,
    // iterator); else if (nn.LearningRateType ==
    // NeuralEnums::LearningRateType::AdamMod) 	return AdamMod(nn, gradient, j,
    // iterator); else if (nn.LearningRateType ==
    ...
    case static_cast<int>(NeuralEnums::LearningRateType::AdaGrad):
      ...
      //	return AdaDelta(nn.Layers[iterator].GradientsLR,
      // nn.Layers[iterator].Parameters, gradient, j) * gradient;
  ...
  __device__ float AdaDelta(float *gradients, float *parameters,
                            float &Gradient, int &j) {
    float result, param;
    gradients[j] =
        momentum * gradients[j] + (1 - momentum) * Gradient * Gradient;
  ...
  __device__ float AdamMod(float &Gradient, int &j, int &iterator) {
    ...
    return (LearningRate * Layers[iterator].Parameters[j] / (1 - beta1)) /
           (sqrt(Layers[iterator].GradientsLR[j] / (1 - beta2)) + epsilon);
  }
  ...
  __device__ float AdaMax(float &gradient, int &j, int &iterator) {
    ...
    // vt
    Layers[iterator].GradientsLR[j] =
        (beta2 * Layers[iterator].GradientsLR[j] > abs(gradient)
             ? beta2 * Layers[iterator].GradientsLR[j]
             : abs(gradient));
  ...
  __device__ float DifferentiateLossWithCuda(float &output, float &target,
                                             int &function, int size) {
    switch (function) {
    case 0:
      return output - target; // NeuralEnums::LossFunctionType::MeanSquaredError
    // case 10: return BinaryCrossentropyDerivative(output, target, size);
    // //NeuralEnums::LossFunctionType::BinaryCrossentropy case 11: return
    // KullbackLeiblerDivergenceDerivative(output, target);
