...
float KullbackLeiblerDivergenceCuda(float *roHat, float &ro, int start,
                                    int end) {
  float sum = 0.0;
...
float MSLCuda(float *output, float *target, int start, int end,
              int outputSize) {
  float Sum = 0;
...
float CalculateLossFunctionCuda(int &function, float *output, float *target,
                                int start, int end, int outputSize) {
  switch (function) {
  case static_cast<int>(NeuralEnums::LossFunctionType::MeanSquaredError):
    return MSLCuda(output, target, start, end, outputSize);
  case static_cast<int>(NeuralEnums::LossFunctionType::BinaryCrossentropy):
    return BinaryCrossentropyCuda(output, target, outputSize);
...
float DifferentiateLossWithCuda(float &output, float &target, int &function,
                                int size) {
  switch (function) {
  case static_cast<int>(NeuralEnums::LossFunctionType::MeanSquaredError):
    return output - target;
  case static_cast<int>(NeuralEnums::LossFunctionType::BinaryCrossentropy):
    return BinaryCrossentropyDerivativeCuda(output, target, size);
  case static_cast<int>(
      NeuralEnums::LossFunctionType::KullbackLeiblerDivergence):
    return KullbackLeiblerDivergenceDerivativeCuda(output, target);
  default:
    break;
  }
}
