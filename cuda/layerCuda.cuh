...
__device__ struct LayerCuda {
  ...
  __device__ inline float &GetNumberFromBatch(float *batch, int batchNumber,
                                              int count) {
  ...
  //---------------------------------------------------
  __device__ void CalculateInputs(int prevLayerSize, float *prevLayerOutputs,
                                  int *prevLayerIndexes,
                                  int &prevLayerIndexVectorSize, int batch,
                                  int start, int end) {
    float result;
  ...
  __device__ void CalculateOutputs(int &batch, int start, int end,
                                   bool training, bool countingRohat) {
    // TODO ჩასამატებელია SoftMax რეალიზაცია
    ActivateWithCuda(&Inputs[batch * Size], &Outputs[batch * Size], IndexVector,
                     start, end, ActivationFunction);
    if (UsingBias)
