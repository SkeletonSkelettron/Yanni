...
inline void gpuAssert(cudaError_t code, const char *file, int line,
                      bool abort = true, bool wait = true) {
  if (code != cudaSuccess) {
...
__global__ void copyDeviceDataElements(float *__restrict__ set,
                                       float *__restrict__ label, int idx,
                                       bool testSet) {
  if (!testSet) {
...
__global__ void checkResults(int maxinputs) {
  ...
  if (idx < maxinputs)
    results[idx] =
        GetMaxIndexCuda(
            neuNet->Layers[neuNet->LayersSize - 1].GetOutputsBatch(idx),
            neuNet->Layers[neuNet->LayersSize - 1].Size) ==
        GetMaxIndexCuda(trainingSetCuda[idx].label,
                        trainingSetCuda[idx].labelSize);
}
...
__global__ void setBatchSize(int batchSize) { neuNet->BatchSize = batchSize; }
...
__global__ void initLayer(int layerIdx, float *weights, float *tempWeights,
                          float *gradients, float *gradientslr,
                          float *parameters, int *idxVec, int *nlIdxVec,
                          int *neur, float *inputsBatch, float *outputsBatch,
                          float *gradientsBatch, float *targetsBatch,
                          int layerType) {
...
// this algorithm is valid if length of input layer <= length of target layer.
// if lt > li other implementation needed
__global__ void assignData(int iter, int maxinputs, bool testing) {
  ...
  if (idx < maxinputs) {
    if (!testing) {
      neuNet->Layers[0].GetOutputsBatch(batch, j) =
          trainingSetCuda[curRow].set[j];

      if (j < neuNet->Layers[neuNet->LayersSize - 1].Size) {
        neuNet->Layers[neuNet->LayersSize - 1].GetTargetsBatch(batch, j) =
            trainingSetCuda[curRow].label[j];
      }
    } else {
...
__global__ void propagateForward(int layerIdx, int maxinputs) {
  ...
  if (idx < maxinputs) {
    neuNet->Layers[layerIdx].CalculateInputs(
        neuNet->Layers[layerIdx - 1].Size, neuNet->Layers[layerIdx - 1].Outputs,
        neuNet->Layers[layerIdx - 1].IndexVectorForNextLayer,
        neuNet->Layers[layerIdx - 1].IndexVectorForNextLayerSize, batch, j,
        j + 1);
    //		neuNet->Layers[layerIdx].CalculateInputs(neuNet->Layers[layerIdx
    //- 1].Size, neuNet->Layers[layerIdx - 1].Outputs, neuNet->Layers[layerIdx -
    // 1].IndexVectorForNextLayer, neuNet->Layers[layerIdx -
    // 1].IndexVectorForNextLayerSize, batch, 0, 1);
...
__global__ void backPropCalcGradients(int layerIdx, int batchCount,
                                      int maxinputs) {
  int idx = blockDim.x * blockIdx.x + threadIdx.x;
  if (idx < maxinputs) {
...
void copyNetrworkCuda(NeuralNetwork &nn, MnistData *trainingSet,
  ...
  ERRCHECK(cudaMemcpy(d_neuralNetwork, &neuNetCuda, sizeof(NeuralNetworkCuda),
                      cudaMemcpyHostToDevice));
  ERRCHECK(cudaMemcpyToSymbol(neuNet, &d_neuralNetwork,
                              sizeof(NeuralNetworkCuda *)));
  ERRCHECK(cudaDeviceSynchronize());
  ...
  for (int i = 0; i < nn.LayersSize; i++) {
    layerCuda[i].ActivationFunction =
        static_cast<int>(nn.Layers[i].ActivationFunction);
    ...
    layerCuda[i].IndexVectorForNextLayerSize =
        nn.Layers[i].IndexVectorForNextLayerSize;
  ...
  int usedMem = sizeof(float) * trainingSetSize * trainingSet[0].setSize +
                ...
                sizeof(float) * testSetSize * testSet[0].labelSize + 1000000000;
  ...
  ERRCHECK(
      cudaMalloc((void **)&d_layerCuda, sizeof(LayerCuda) * nn.LayersSize));
  ERRCHECK(cudaMemcpy(d_layerCuda, layerCuda, sizeof(LayerCuda) * nn.LayersSize,
                      cudaMemcpyHostToDevice));
  ...
  ERRCHECK(cudaMemcpy(d_results, h_results, sizeof(int) * maxBatchSize,
                      cudaMemcpyHostToDevice));
  copyLayers<<<1, 1>>>(maxBatchSize, d_results);
  ERRCHECK(cudaDeviceSynchronize());

  float *d_weights = NULL, *d_gradients = NULL, *d_gradientslr = NULL,
        *d_parameters = NULL;
  float *h_weights = NULL, *h_gradients = NULL, *h_gradientslr = NULL,
        *h_parameters = NULL;
  float *h_tempWeights = NULL, *d_tempWeights = NULL;
  float *d_inputsBatch = NULL, *d_gradientsBatch = NULL, *d_outputsBatch = NULL,
        *d_targetsBatch = NULL;
  float *h_inputsBatch = NULL, *h_gradientsBatch = NULL, *h_outputsBatch = NULL,
        *h_targetsBatch = NULL;
  float *d_inputsBatchHptr = NULL, *d_gradientsBatchHptr = NULL,
        *d_outputsBatchHptr = NULL, *d_targetsBatchHptr = NULL;
  int *d_idxVec = NULL;
  int *d_nlIdxVec = NULL;
  ...
  for (int i = 0; i < nn.LayersSize; i++) {
    ...
    ERRCHECK(cudaMemcpy(d_gradients, h_gradients, sizeof(float) * weightsSize,
                        cudaMemcpyHostToDevice));

    ERRCHECK(cudaMalloc((void **)&d_gradientslr, sizeof(float) * weightsSize));
    h_gradientslr = new float[weightsSize]{};
    ERRCHECK(cudaMemcpy(d_gradientslr, h_gradientslr,
                        sizeof(float) * weightsSize, cudaMemcpyHostToDevice));

    ERRCHECK(cudaMalloc((void **)&d_parameters, sizeof(float) * weightsSize));
    h_parameters = new float[weightsSize]{};
    ERRCHECK(cudaMemcpy(d_parameters, h_parameters, sizeof(float) * weightsSize,
                        cudaMemcpyHostToDevice));

    ERRCHECK(cudaMalloc((void **)&d_tempWeights, sizeof(float) * weightsSize));
    h_tempWeights = new float[weightsSize]{};
    ERRCHECK(cudaMemcpy(d_tempWeights, h_tempWeights,
                        sizeof(float) * weightsSize, cudaMemcpyHostToDevice));

    ERRCHECK(cudaMalloc((void **)&d_weights, sizeof(float) * weightsSize));
    ERRCHECK(cudaMemcpy(d_weights, nn.Layers[i].Weights,
                        sizeof(float) * weightsSize, cudaMemcpyHostToDevice));

    ERRCHECK(cudaMalloc((void **)&d_idxVec,
                        sizeof(int) * nn.Layers[i].IndexVectorSize));
    ERRCHECK(cudaMemcpy(d_idxVec, nn.Layers[i].IndexVector,
                        sizeof(int) * nn.Layers[i].IndexVectorSize,
                        cudaMemcpyHostToDevice));

    ERRCHECK(
        cudaMalloc((void **)&d_nlIdxVec,
                   sizeof(int) * nn.Layers[i].IndexVectorForNextLayerSize));
    ERRCHECK(cudaMemcpy(d_nlIdxVec, nn.Layers[i].IndexVectorForNextLayer,
                        sizeof(int) * nn.Layers[i].IndexVectorForNextLayerSize,
                        cudaMemcpyHostToDevice));
    ...
    ERRCHECK(
        cudaMemcpy(d_neur, h_neur, sizeof(int) * size, cudaMemcpyHostToDevice));
    ...
    for (int hi = batchSize; hi--;)
      h_inputsBatch[hi * nn.Layers[i].Size] =
          nn.Layers[i].UsingBias &&
                  nn.Layers[i].LayerType == NeuralEnums::LayerType::HiddenLayer
              ? 1.0
              : 0.0;
    ERRCHECK(cudaMemcpy(d_inputsBatch, h_inputsBatch, batchvecAlocSize,
                        cudaMemcpyHostToDevice));

    ERRCHECK(cudaMalloc((void **)&d_outputsBatch, batchvecAlocSize));
    h_outputsBatch = new float[batchvecSize]{};
    ERRCHECK(cudaMemcpy(d_outputsBatch, h_outputsBatch, batchvecAlocSize,
                        cudaMemcpyHostToDevice));

    ERRCHECK(cudaMalloc((void **)&d_gradientsBatch, batchvecAlocSize));
    h_gradientsBatch = new float[batchvecSize]{};
    ERRCHECK(cudaMemcpy(d_gradientsBatch, h_gradientsBatch, batchvecAlocSize,
                        cudaMemcpyHostToDevice));

    if (nn.Layers[i].LayerType == NeuralEnums::LayerType::OutputLayer) {
      ERRCHECK(cudaMalloc((void **)&d_targetsBatch, batchvecAlocSize));
      h_targetsBatch = new float[batchvecSize]{};
      ERRCHECK(cudaMemcpy(d_targetsBatch, h_targetsBatch, batchvecAlocSize,
                          cudaMemcpyHostToDevice));
    }
    initLayer<<<1, 1>>>(i, d_weights, d_tempWeights, d_gradients, d_gradientslr,
                        d_parameters, d_idxVec, d_nlIdxVec, d_neur,
                        d_inputsBatch, d_outputsBatch, d_gradientsBatch,
                        d_targetsBatch,
  ...
  ERRCHECK(cudaMemcpy(d_mnist, trainingSet, sizeof(MnistData) * trainingSetSize,
                      cudaMemcpyHostToDevice));
  ...
  ERRCHECK(cudaMemcpy(d_mnistTest, testSet, sizeof(MnistData) * testSetSize,
                      cudaMemcpyHostToDevice));
  ...
  if (notTesting) {
    for (int i = 0; i < trainingSetSize; i++) {
      ERRCHECK(
          cudaMalloc((void **)&d_set, sizeof(float) * trainingSet[i].setSize));
      ERRCHECK(cudaMalloc((void **)&d_label,
                          sizeof(float) * trainingSet[i].labelSize));
      ERRCHECK(cudaMemcpy(d_set, trainingSet[i].set,
                          sizeof(float) * trainingSet[i].setSize,
                          cudaMemcpyHostToDevice));
      ERRCHECK(cudaMemcpy(d_label, trainingSet[i].label,
                          sizeof(float) * trainingSet[i].labelSize,
                          cudaMemcpyHostToDevice));
    ...
    for (int i = 0; i < testSetSize; i++) {
      ERRCHECK(
          cudaMalloc((void **)&d_testset, sizeof(float) * testSet[i].setSize));
      ERRCHECK(cudaMalloc((void **)&d_testlabel,
                          sizeof(float) * testSet[i].labelSize));
      ERRCHECK(cudaMemcpy(d_testset, testSet[i].set,
                          sizeof(float) * testSet[i].setSize,
                          cudaMemcpyHostToDevice));
      ERRCHECK(cudaMemcpy(d_testlabel, testSet[i].label,
                          sizeof(float) * testSet[i].labelSize,
                          cudaMemcpyHostToDevice));
  ...
  for (int globalEpochs = 0; globalEpochs < globalEpochsCount; globalEpochs++) {
    clock_t start = clock();
    int blocksPerGrid =
        (nn.BatchSize * nn.Layers[0].Size + threadsPerBlock - 1) /
        threadsPerBlock;
    ...
    for (int i = 0; i < trainingSetSize / nn.BatchSize - 1; i++) {
      ...
      for (int layerIdx = 1; layerIdx < nn.LayersSize; layerIdx++) {
        ...
        propagateForward<<<blocksPerGrid, threadsPerBlock>>>(layerIdx,
                                                             maxinputs);
      ...
      for (int layerIdx = nn.LayersSize - 1; layerIdx >= 1; layerIdx--) {
        ...
        if (notTesting) {
          maxinputs = nn.Layers[layerIdx - 1].IndexVectorForNextLayerSize *
                      nn.Layers[layerIdx].IndexVectorSize;
          blocksPerGrid = (maxinputs + threadsPerBlock - 1) / threadsPerBlock;
          backPropCalcGradients<<<blocksPerGrid, threadsPerBlock>>>(
              layerIdx, nn.BatchSize, maxinputs);
        }
      }
    }
    checkLayers<<<1, 1>>>();
    ...
    blocksPerGrid = (maxBatchSize * nn.Layers[0].Size + threadsPerBlock - 1) /
                    threadsPerBlock;
    maxinputs = nn.Layers[0].Size * maxBatchSize;
    for (int i = 0;
         i < (testSetSize <= maxBatchSize ? 1 : maxBatchSize / testSetSize);
         i++) //  maxBatchSize % testSetSize !=0 case
    {
    ...
    ERRCHECK(cudaDeviceSynchronize());
    clock_t end = clock();
    std::cout << "Epoch: " << globalEpochs << " completed in "
              << (float(end - start) / CLOCKS_PER_SEC)
              << " seconds. training result: " << passAccuracy << std::endl;
